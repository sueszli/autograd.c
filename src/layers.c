...
Layer *layer_linear_create(uint64_t in_features, uint64_t features_out, bool bias) {
    ...
    uint64_t w_shape[] = {in_features, features_out};
    uint64_t w_size = in_features * features_out;
...
static void dropout_free(Layer *layer) { free(layer); }
