...
Tensor *tensor_reshape(const Tensor *t, const int64_t *new_shape, int64_t new_ndim) {
    ...
    memcpy(resolved_shape, new_shape, (size_t)new_ndim * sizeof(int64_t));
...
Tensor *tensor_sum(Tensor *t, int64_t axis, bool keepdims) {
    ...
    int64_t new_ndim = keepdims ? t->ndim : t->ndim - 1;

    if (new_ndim > 0) {
        new_shape = (int64_t *)malloc((size_t)new_ndim * sizeof(int64_t));
